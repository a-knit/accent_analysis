...
def plot_nth_word(X, y, n, f_per_word):
    ...
    X_list = [X[y=='other'], X[y=='european'], X[y=='indo_iranian'], X[y=='sino_tibetan']]
    ...
    for i in xrange(4):
        ...
        plt.scatter(F1, F2, alpha=.5, color=color)

    plt.title('Mean Formant Values for [a:] from "call"')
    plt.show()
...
def plot_n_words(X, y, n, f_per_word):
    ...
    color_list = ['fuchsia', 'slateblue', 'orange', 'olive']
    ...
    plt.title('Mean Formant Values for Vowels from First %d words' % n)
    plt.show()
